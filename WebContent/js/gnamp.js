...
page = function(url, param, callback) {
	pageConfig = {
		...
		previous : function() {
			pageConfig.gotoPage(pageConfig.currentPageIndex - 1 <= 1 ? 1
					: pageConfig.currentPageIndex - 1);
		},
		next : function() {
			pageConfig
					.gotoPage(pageConfig.currentPageIndex + 1 >= pageConfig.totalPageIndex ? pageConfig.totalPageIndex
							: pageConfig.currentPageIndex + 1);
		},
		last : function() {
		...
		ajax : function(ajaxParam) {
			$.ajax({
				...
				url : this.url,
				/**
				 * 直接指定param那么就可以。用globeParam，这样吧不用关心其他属性的更改，在globeParam里面会自己寻找
				 */
				data : ajaxParam || pageConfig.globeParam(pageConfig.param),
				complete : function() {
				},
				success : function(msg) {
					pageConfig.param = ajaxParam || (pageConfig.param || {});
					if (!msg.success) {
		...
		setTotalFlag : function(flag) {
			...
			$("#" + pageConfig.currentWithTotalID).html(
					pageConfig.currentPageIndex + "/"
							+ pageConfig.totalPageIndex);
			$("#" + pageConfig.pageIndexID).html(
					pageConfig.refreshGotoOption(pageConfig.totalPageIndex));
		...
		refreshGotoOption : function(length) {
			if (length <= 1) {
				$("select[name=" + pageConfig.pageIndexID + "] option")
						.remove();
				$("select[name=" + pageConfig.pageIndexID + "]").append(
			...
			$("select[name=" + pageConfig.pageIndexID + "]").val(
					pageConfig.currentPageIndex);
		},
