...
(function(l) {
	var D, E, F, G, H, I, p = {}, J = {}, s = {}, M = 0, K = {
		...
		callback : {
			...
			onRemove : null
				...
				a.bind(c.NODECREATED, function(a, c, g) {
							k.apply(b.callback.onNodeCreated, [a, c, g])
						});
				a.unbind(c.CLICK);
				a.bind(c.CLICK, function(a, c, g, j, f) {
							k.apply(b.callback.onClick, [c, g, j, f])
						});
				a.unbind(c.EXPAND);
				a.bind(c.EXPAND, function(a, c, g) {
							k.apply(b.callback.onExpand, [a, c, g])
						});
				a.unbind(c.COLLAPSE);
				a.bind(c.COLLAPSE, function(a, c, g) {
							k.apply(b.callback.onCollapse, [a, c, g])
						});
				a.unbind(c.ASYNC_SUCCESS);
				a.bind(c.ASYNC_SUCCESS, function(a, c, g, j) {
							k.apply(b.callback.onAsyncSuccess, [a, c, g, j])
						});
				a.unbind(c.ASYNC_ERROR);
				a.bind(c.ASYNC_ERROR, function(a, c, g, j, f, h) {
							k
		...
		var a = b.target, c = p[b.data.treeId], d = "", e = null, g = "", j = "", i = null, m = null, l = null;
		if (k.eqs(b.type, "mousedown"))
			j = "mousedown";
		else if (k.eqs(b.type, "mouseup"))
			j = "mouseup";
		else if (k.eqs(b.type, "contextmenu"))
			j = "contextmenu";
		else if (k.eqs(b.type, "click"))
			if (k.eqs(a.tagName, "span")
					&& a.getAttribute("treeNode" + f.id.SWITCH) !== null)
				d = a.parentNode.id, g = "switchNode";
			else {
				if (l = k.getMDom(c, a, [{
									tagName : "a",
									attrName : "treeNode" + f.id.A
								}]))
					d = l.parentNode.id, g = "clickNode"
			}
		else if (k.eqs(b.type, "dblclick")
				&& (j = "dblclick", l = k.getMDom(c, a, [{
									tagName : "a",
									attrName : "treeNode" + f.id.A
								}])))
			d = l.parentNode.id, g = "switchNode";
		if (j.length > 0 && d.length == 0 && (l = k.getMDom(c, a, [{
							tagName : "a",
							attrName : "treeNode" + f.id.A
						}])))
			d = l.parentNode.id;
		if (d.length > 0)
			switch (e = h.getNodeCache(c, d), g) {
				case "switchNode" :
					e.isParent
							? k.eqs(b.type, "click")
									|| k.eqs(b.type, "dblclick")
									&& k.apply(c.view.dblClickExpand, [
													c.treeId, e],
		...
		getNextNode : function(b, a) {
			if (!a)
				return null;
			var c = b.data.key.children, d = a.parentTId
					? a.getParentNode()
					: h.getRoot(b);
			if (!a.isLastNode)
				if (a.isFirstNode)
					...
					for (var e = 1, g = d[c].length - 1; e < g; e++)
						if (d[c][e] === a)
							return d[c][e + 1];
			return null
		},
		getNodeByParam : function(b, a, c, d) {
			if (!a || !c)
				return null;
			for (var e = b.data.key.children, g = 0, j = a.length; g < j; g++) {
				if (a[g][c] == d)
					return a[g];
		...
		getNodesByParam : function(b, a, c, d) {
			if (!a || !c)
				return [];
			for (var e = b.data.key.children, g = [], j = 0, f = a.length; j < f; j++)
				a[j][c] == d && g.push(a[j]), g = g.concat(h.getNodesByParam(b,
						a[j][e], c, d));
			return g
		},
		getNodesByParamFuzzy : function(b, a, c, d) {
			if (!a || !c)
				return [];
			for (var e = b.data.key.children, g = [], j = 0, f = a.length; j < f; j++)
				typeof a[j][c] == "string" && a[j][c].indexOf(d) > -1
						&& g.push(a[j]), g = g.concat(h.getNodesByParamFuzzy(b,
						a[j][e], c, d));
			return g
		},
		getNodesByFilter : function(b, a, c, d) {
			if (!a)
				return d ? null : [];
			for (var e = b.data.key.children, g = d ? null : [], j = 0, f = a.length; j < f; j++) {
				if (k.apply(c, [a[j]], !1)) {
					if (d)
						return a[j];
					g.push(a[j])
				}
				var i = h.getNodesByFilter(b, a[j][e], c, d);
				if (d && i)
					return i;
				g = d ? i : g.concat(i)
			}
		...
		removeNodeCache : function(b, a) {
			...
			delete h.getCache(b).nodes[a.tId]
		},
		...
		transformTozTreeFormat : function(b, a) {
			var c, d, e = b.data.simpleData.idKey, g = b.data.simpleData.pIdKey, j = b.data.key.children;
			if (!e || e == "" || !a)
				return [];
			if (k.isArray(a)) {
				var f = [], i = [];
		...
		bindTree : function(b) {
			...
			b.unbind("click", n.proxy);
			b.bind("click", a, n.proxy);
			b.unbind("dblclick", n.proxy);
			b.bind("dblclick", a, n.proxy);
			b.unbind("mouseover", n.proxy);
			b.bind("mouseover", a, n.proxy);
			b.unbind("mouseout", n.proxy);
			b.bind("mouseout", a, n.proxy);
			b.unbind("mousedown", n.proxy);
			b.bind("mousedown", a, n.proxy);
			b.unbind("mouseup", n.proxy);
			b.bind("mouseup", a, n.proxy);
			b.unbind("contextmenu", n.proxy);
			b.bind("contextmenu", a, n.proxy)
		},
		doProxy : function(b) {
		...
		proxy : function(b) {
			...
			if (!k.uCanDo(a, b))
				return !0;
			for (var c = n.doProxy(b), d = !0, e = !1, g = 0, j = c.length; g < j; g++) {
				var f = c[g];
				f.nodeEventCallback
						&& (e = !0, d = f.nodeEventCallback.apply(f,
								[b, f.node])
								&& d);
				f.treeEventCallback
						&& (e = !0, d = f.treeEventCallback.apply(f,
								[b, f.node])
								&& d)
			}
			try {
	...
	D = function(b, a) {
		...
		if (a.open) {
			if (k.apply(c.callback.beforeCollapse, [c.treeId, a], !0) == !1)
				return !0
		} else if (k.apply(c.callback.beforeExpand, [c.treeId, a], !0) == !1)
			return !0;
	...
	E = function(b, a) {
		var c = p[b.data.treeId], d = c.view.autoCancelSelected && b.ctrlKey
				&& h.isSelectedNode(c, a) ? 0 : c.view.autoCancelSelected
				&& b.ctrlKey && c.view.selectedMulti ? 2 : 1;
	...
	F = function(b, a) {
		var c = p[b.data.treeId];
		k.apply(c.callback.beforeMouseDown, [c.treeId, a], !0)
				&& k.apply(c.callback.onMouseDown, [b, c.treeId, a]);
		return !0
	};
	G = function(b, a) {
		var c = p[b.data.treeId];
		k.apply(c.callback.beforeMouseUp, [c.treeId, a], !0)
				&& k.apply(c.callback.onMouseUp, [b, c.treeId, a]);
		return !0
	};
	H = function(b, a) {
		var c = p[b.data.treeId];
		k.apply(c.callback.beforeDblClick, [c.treeId, a], !0)
				&& k.apply(c.callback.onDblClick, [b, c.treeId, a]);
		return !0
	};
	I = function(b, a) {
		var c = p[b.data.treeId];
		k.apply(c.callback.beforeRightClick, [c.treeId, a], !0)
				&& k.apply(c.callback.onRightClick, [b, c.treeId, a]);
	...
	var k = {
		...
		getMDom : function(b, a, c) {
			...
			for (; a && a.id !== b.treeId;) {
				for (var d = 0, e = c.length; a.tagName && d < e; d++)
					if (k.eqs(a.tagName, c[d].tagName)
		...
		addNodes : function(b, a, c, d) {
			if (!b.data.keep.leaf || !a || a.isParent)
				if (k.isArray(c) || (c = [c]), b.data.simpleData.enable
						&& (c = h.transformTozTreeFormat(b, c)), a) {
					var e = l("#" + a.tId + f.id.SWITCH), g = l("#" + a.tId
							+ f.id.ICON), j = l("#" + a.tId + f.id.UL);
					if (!a.open)
						i.replaceSwitchClass(a, e, f.folder.CLOSE), i
								.replaceIcoClass(a, g, f.folder.CLOSE), a.open = !1, j
								.css({
											display : "none"
										});
		...
		appendNodes : function(b, a, c, d, e, g) {
			if (!c)
				return [];
			for (var j = [], l = b.data.key.children, m = b.data.key.name, n = h
					.getTitleKey(b), p = 0, N = c.length; p < N; p++) {
				var o = c[p], u = (d ? d : h.getRoot(b))[l].length == c.length
						&& p == 0, r = p == c.length - 1;
				e && (h.initNode(b, a, o, d, u, r, g), h.addNodeCache(b, o));
				u = [];
				o[l]
						&& o[l].length > 0
						&& (u = i
								.appendNodes(b, a + 1, o[l], o, e, g && o.open));
				if (g) {
					var r = i.makeNodeUrl(b, o), s = i.makeNodeFontCss(b, o), t = [], q;
					for (q in s)
						t.push(q, ":", s[q], ";");
					j.push("<li id='", o.tId, "' class='level", o.level,
							"' tabindex='0' hidefocus='true' treenode>",
							"<span id='", o.tId, f.id.SWITCH,
							"' title='' class='", i.makeNodeLineClass(b, o),
							"' treeNode", f.id.SWITCH, "></span>");
					h.getBeforeA(b, o, j);
					j.push("<a id='", o.tId, f.id.A, "' class='level", o.level,
							"' treeNode", f.id.A, ' onclick="', o.click || "",
							'" ', r != null && r.length > 0 ? "href='" + r
									+ "'" : "", " target='", i
									.makeNodeTarget(o), "' style='",
							t.join(""), "'");
					k.apply(b.view.showTitle, [b.treeId, o], b.view.showTitle)
							&& o[n]
							&& j.push("title='", o[n].replace(/'/g, "&#39;")
											.replace(/</g, "&lt;").replace(
													/>/g, "&gt;"), "'");
					j.push(">");
					h.getInnerBeforeA(b, o, j);
					r = b.view.nameIsHTML ? o[m] : o[m].replace(/&/g, "&amp;")
							.replace(/</g, "&lt;").replace(/>/g, "&gt;");
					j.push("<span id='", o.tId, f.id.ICON,
							"' title='' treeNode", f.id.ICON, " class='", i
									.makeNodeIcoClass(b, o), "' style='", i
									.makeNodeIcoStyle(b, o),
							"'></span><span id='", o.tId, f.id.SPAN, "'>", r,
							"</span>");
					h.getInnerAfterA(b, o, j);
					j.push("</a>");
					h.getAfterA(b, o, j);
					o.isParent && o.open && i.makeUlHtml(b, o, j, u.join(""));
					j.push("</li>");
		...
		appendParentULDom : function(b, a) {
			var c = [], d = l("#" + a.tId), e = l("#" + a.tId + f.id.UL), g = i
					.appendNodes(b, a.level + 1, a[b.data.key.children], a, !1,
							!0);
			i.makeUlHtml(b, a, c, g.join(""));
			!d.get(0)
					&& a.parentTId
					&& (i.appendParentULDom(b, a.getParentNode()), d = l("#"
							+ a.tId));
			e.get(0) && e.remove();
			d.append(c.join(""));
		...
		asyncNode : function(b, a, c, d) {
			...
			if (a && !a.isParent)
				...
				return !1;
			else if (k.apply(b.callback.beforeAsync, [b.treeId, a], !0) == !1)
				return k.apply(d), !1;
			if (a)
				a.isAjaxing = !0, l("#" + a.tId + f.id.ICON).attr({
							style : "",
							"class" : "button ico_loading"
						});
			var j = b.async.contentType == "application/json", h = j ? "{" : "", m = "";
			for (e = 0, g = b.async.autoParam.length; a && e < g; e++) {
				var n = b.async.autoParam[e].split("="), p = n;
				n.length > 1 && (p = n[1], n = n[0]);
				j
						? (m = typeof a[n] == "string" ? '"' : "", h += '"' + p
								+ ('":' + m + a[n]).replace(/'/g, "\\'") + m
								+ ",")
						: h += p + ("=" + a[n]).replace(/&/g, "%26") + "&"
			}
			if (k.isArray(b.async.otherParam))
				for (e = 0, g = b.async.otherParam.length; e < g; e += 2)
					j
							? (m = typeof b.async.otherParam[e + 1] == "string"
									? '"'
									: "", h += '"'
									+ b.async.otherParam[e]
									+ ('":' + m + b.async.otherParam[e + 1])
											.replace(/'/g, "\\'") + m + ",")
							: h += b.async.otherParam[e]
									+ ("=" + b.async.otherParam[e + 1])
											.replace(/&/g, "%26") + "&";
			else
				for (var q in b.async.otherParam)
					j ? (m = typeof b.async.otherParam[q] == "string"
							? '"'
							: "", h += '"'
							+ q
							+ ('":' + m + b.async.otherParam[q]).replace(/'/g,
									"\\'") + m + ",") : h += q
							+ ("=" + b.async.otherParam[q])
									.replace(/&/g, "%26") + "&";
			h.length > 1 && (h = h.substring(0, h.length - 1));
			j && (h += "}");
			l.ajax({
						...
						success : function(e) {
							...
							g && g != "" ? (g = k.apply(b.async.dataFilter, [
											b.treeId, a, g], g), i.addNodes(b,
									a, g ? k.clone(g) : [], !!c)) : i.addNodes(
									b, a, [], !!c);
							b.treeObj.trigger(f.event.ASYNC_SUCCESS, [b.treeId,
											a, e]);
							k.apply(d)
						},
						error : function(c, d, e) {
							if (a)
								a.isAjaxing = null;
							i.setNodeLineIcos(b, a);
							b.treeObj.trigger(f.event.ASYNC_ERROR, [b.treeId,
											a, c, d, e])
						}
					});
			return !0
		...
		cancelPreSelectedNode : function(b, a) {
			for (var c = h.getRoot(b).curSelectedList, d = c.length - 1; d >= 0; d--)
				if (!a || a === c[d])
					if (l("#" + c[d].tId + f.id.A)
							.removeClass(f.node.CURSELECTED), i.setNodeName(b,
							c[d]), a) {
		...
		createNodes : function(b, a, c, d) {
			if (c && c.length != 0) {
				var e = h.getRoot(b), g = b.data.key.children, g = !d || d.open
						|| !!l("#" + d[g][0].tId).get(0);
				e.createdNodes = [];
				a = i.appendNodes(b, a, c, d, !0, g);
				d ? (d = l("#" + d.tId + f.id.UL), d.get(0)
						&& d.append(a.join(""))) : b.treeObj.append(a.join(""));
				i.createNodeCallback(b)
			}
		},
		expandCollapseNode : function(b, a, c, d, e) {
			...
			if (a) {
				...
				if (a.open == c)
					...
					!a.open
							&& a.isParent
							&& (!l("#" + a.tId + f.id.UL).get(0) || a[j]
									&& a[j].length > 0
									&& !l("#" + a[j][0].tId).get(0))
							&& i.appendParentULDom(b, a);
					var c = l("#" + a.tId + f.id.UL), g = l("#" + a.tId
							+ f.id.SWITCH), m = l("#" + a.tId + f.id.ICON);
					a.isParent
		...
		expandCollapseSonNode : function(b, a, c, d, e) {
			var g = h.getRoot(b), f = b.data.key.children, g = a ? a[f] : g[f], f = a
					? !1
					: d, k = h.getRoot(b).expandTriggerFlag;
			h.getRoot(b).expandTriggerFlag = !1;
			if (g)
				for (var l = 0, n = g.length; l < n; l++)
					g[l] && i.expandCollapseSonNode(b, g[l], c, f);
			h.getRoot(b).expandTriggerFlag = k;
			i.expandCollapseNode(b, a, c, d, e)
		},
		...
		makeNodeIcoStyle : function(b, a) {
			...
			if (!a.isAjaxing) {
				...
				(b.view.showIcon == !1 || !k.apply(b.view.showIcon, [b.treeId,
								a], !0))
		...
		makeUlHtml : function(b, a, c, d) {
			c.push("<ul id='", a.tId, f.id.UL, "' class='level", a.level, " ",
					i.makeUlLineClass(b, a), "' style='display:", a.open
							? "block"
							: "none", "'>");
			c.push(d);
			c.push("</ul>")
		...
		removeChildNodes : function(b, a) {
			if (a) {
				...
				if (d) {
					...
					delete a[c];
					b.data.keep.parent
							? l("#" + a.tId + f.id.UL).empty()
							: (a.isParent = !1, a.open = !1, c = l("#" + a.tId
									+ f.id.SWITCH), d = l("#" + a.tId
									+ f.id.ICON), i.replaceSwitchClass(a, c,
									f.folder.DOCU), i.replaceIcoClass(a, d,
									f.folder.DOCU), l("#" + a.tId + f.id.UL)
									.remove())
				}
			}
		},
		removeNode : function(b, a) {
			...
			var k;
			if (!b.data.keep.parent && e[d].length < 1)
				e.isParent = !1, e.open = !1, g = l("#" + e.tId + f.id.UL), j = l("#"
						+ e.tId + f.id.SWITCH), k = l("#" + e.tId + f.id.ICON), i
						.replaceSwitchClass(e, j, f.folder.DOCU), i
						.replaceIcoClass(e, k, f.folder.DOCU), g.css("display",
						"none");
			else if (b.view.showLine && e[d].length > 0) {
				var m = e[d][e[d].length - 1];
				m.isLastNode = !0;
				m.isFirstNode = e[d].length == 1;
				g = l("#" + m.tId + f.id.UL);
				j = l("#" + m.tId + f.id.SWITCH);
				k = l("#" + m.tId + f.id.ICON);
				e == c
						? e[d].length == 1 ? i.replaceSwitchClass(m, j,
								f.line.ROOT) : (c = l("#" + e[d][0].tId
								+ f.id.SWITCH), i.replaceSwitchClass(e[d][0],
								c, f.line.ROOTS), i.replaceSwitchClass(m, j,
								f.line.BOTTOM))
						: i.replaceSwitchClass(m, j, f.line.BOTTOM);
		...
		setNodeLineIcos : function(b, a) {
			if (a) {
				var c = l("#" + a.tId + f.id.SWITCH), d = l("#" + a.tId
						+ f.id.UL), e = l("#" + a.tId + f.id.ICON), g = i
						.makeUlLineClass(b, a);
		...
		setNodeName : function(b, a) {
			var c = b.data.key.name, d = h.getTitleKey(b), e = l("#" + a.tId
					+ f.id.SPAN);
			e.empty();
			b.view.nameIsHTML ? e.html(a[c]) : e.text(a[c]);
			k.apply(b.view.showTitle, [b.treeId, a], b.view.showTitle) && a[d]
					&& l("#" + a.tId + f.id.A).attr("title", a[d])
		},
	...
	l.fn.zTree = {
		...
		init : function(b, a, c) {
			...
			d.treeObj.empty();
			p[d.treeId] = d;
			if (l.browser.msie && parseInt(l.browser.version) < 7)
				d.view.expandSpeed = "";
			h.initRoot(d);
			b = h.getRoot(d);
			...
			c = {
				...
				addNodes : function(a, b, c) {
					...
					if (a && !a.isParent && d.data.keep.leaf)
						return null;
					var h = k.clone(k.isArray(b) ? b : [b]);
					k.canAsync(d, a) ? i.asyncNode(d, a, c, f) : f();
					return h
				},
				...
				expandNode : function(a, b, c, f, m) {
					...
					if ((m = !!m)
							&& b
							&& k.apply(d.callback.beforeExpand, [d.treeId, a],
									!0) == !1)
						return null;
					else if (m
							&& !b
							&& k.apply(d.callback.beforeCollapse,
									[d.treeId, a], !0) == !1)
						return null;
					b
							&& a.parentTId
							&& i.expandCollapseParentNode(this.setting, a
											.getParentNode(), b, !1);
					if (b === a.open && !c)
						return null;
					h.getRoot(d).expandTriggerFlag = m;
					c
							? i.expandCollapseSonNode(this.setting, a, b, !0,
									function() {
										f !== !1
												&& l("#" + a.tId).focus()
														.blur()
									})
							: (a.open = !b, i.switchNode(this.setting, a), f !== !1
									&& l("#" + a.tId).focus().blur());
					return b
				},
				getNodes : function() {
				...
				getNodeByParam : function(a, b, c) {
					return !a ? null : h.getNodeByParam(this.setting, c
									? c[this.setting.data.key.children]
									: h.getNodes(this.setting), a, b)
				},
				getNodeByTId : function(a) {
					return h.getNodeCache(this.setting, a)
				},
				getNodesByParam : function(a, b, c) {
					return !a ? null : h.getNodesByParam(this.setting, c
									? c[this.setting.data.key.children]
									: h.getNodes(this.setting), a, b)
				},
				getNodesByParamFuzzy : function(a, b, c) {
					return !a ? null : h.getNodesByParamFuzzy(this.setting, c
									? c[this.setting.data.key.children]
									: h.getNodes(this.setting), a, b)
				},
				getNodesByFilter : function(a, b, c) {
					b = !!b;
					return !a || typeof a != "function" ? b ? null : [] : h
							.getNodesByFilter(this.setting, c
											? c[this.setting.data.key.children]
											: h.getNodes(this.setting), a, b)
				},
				getNodeIndex : function(a) {
					...
					for (var b = d.data.key.children, c = a.parentTId ? a
							.getParentNode() : h.getRoot(this.setting), f = 0, i = c[b].length; f < i; f++)
						if (c[b][f] == a)
							return f;
					return -1
				},
				getSelectedNodes : function() {
					for (var a = [], b = h.getRoot(this.setting).curSelectedList, c = 0, d = b.length; c < d; c++)
						a.push(b[c]);
					return a
				},
				isSelectedNode : function(a) {
					return h.isSelectedNode(this.setting, a)
				},
				reAsyncChildNodes : function(a, b, c) {
					if (this.setting.async.enable) {
						var d = !a;
						d && (a = h.getRoot(this.setting));
						b == "refresh"
								&& (a[this.setting.data.key.children] = [], d
				...
				removeNode : function(a, b) {
					a
							&& (b = !!b, b
									&& k.apply(d.callback.beforeRemove, [
													d.treeId, a], !0) == !1
									|| (i.removeNode(d, a), b
				...
				selectNode : function(a, b) {
					a
							...
							&& (b = d.view.selectedMulti && b, a.parentTId ? i
									.expandCollapseParentNode(this.setting, a
													.getParentNode(), !0, !1,
											function() {
												l("#" + a.tId).focus().blur()
											}) : l("#" + a.tId).focus().blur(), i
									.selectNode(this.setting, a, b))
				},
				...
				updateNode : function(a) {
					a
							&& l("#" + a.tId).get(0)
							&& k.uCanDo(this.setting)
							&& (i.setNodeName(this.setting, a), i
									.setNodeTarget(a), i.setNodeUrl(
									this.setting, a), i.setNodeLineIcos(
									this.setting, a), i.setNodeFontCss(
									this.setting, a))
				}
			};
			b.treeTools = c;
