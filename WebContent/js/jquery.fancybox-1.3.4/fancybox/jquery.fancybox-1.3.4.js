 ...
 * 
 * Copyright (c) 2008 - 2010 Janis Skarnelis That said, it is hardly a
 * one-person project. Many people have submitted bugs, code, and offered their
 * advice freely. Their support is greatly appreciated.
 * 
...
(function($) {
		...
		switch (type) {
			...
			case 'inline' :
				...
				$('<div class="fancybox-inline-tmp" />').hide()
						.insertBefore($(obj)).bind('fancybox-cleanup',
								function() {
									$(this).replaceWith(content.children());
							...
							success : function(data, textStatus, XMLHttpRequest) {
								...
								if (o.status == 200) {
									if (typeof selectedOpts.ajax.win == 'function') {
										ret = selectedOpts.ajax.win(href, data,
	...
	_show = function() {
		...
		if (currentOpts.overlayShow) {
			overlay.css({
						...
						'cursor' : currentOpts.hideOnOverlayClick
								? 'pointer'
								: 'auto',
	...
	_process_title = function() {
		...
		titleStr = $.isFunction(currentOpts.titleFormat)
				? currentOpts.titleFormat(titleStr, currentArray, currentIndex,
						currentOpts)
		...
		title.addClass('fancybox-title-' + currentOpts.titlePosition)
				.html(titleStr).appendTo('body').show();
		...
		switch (currentOpts.titlePosition) {
			...
			case 'float' :
				title
						.css(
								'left',
								parseInt((title.width() - final_pos.width - 40)
												/ 2, 10)
										* -1).appendTo(wrap);
				break;
	...
	_finish = function() {
		...
		if (currentOpts.type == 'iframe') {
			$('<iframe id="fancybox-frame" name="fancybox-frame'
					...
					+ ' scrolling="' + selectedOpts.scrolling + '" src="'
	...
	_get_zoom_to = function() {
		var view = _get_viewport(), to = {}, resize = currentOpts.autoScale, double_padding = currentOpts.padding
				* 2, ratio;
		...
		if (currentOpts.height.toString().indexOf('%') > -1) {
			to.height = parseInt((view[1] * parseFloat(currentOpts.height))
							/ 100, 10);
		} else {
	...
	$.fancybox.pos = function(pos) {
		...
		if (pos > -1 && pos < currentArray.length) {
			...
			selectedIndex = pos >= currentArray.length
					? 0
	...
	$.fancybox.close = function() {
		...
		if (currentOpts.transitionOut == 'elastic') {
			...
			wrap.fadeOut(currentOpts.transitionOut == 'none'
							? 0
			...
			wrap.bind('mousewheel.fb', function(e, delta) {
						if (busy) {
							e.preventDefault();
								...
								|| $(e.target).get(0).scrollHeight === $(e.target)
										.get(0).clientHeight) {
							e.preventDefault();
