...
(function(b) {
	var m, t, u, f, D, j, E, n, z, A, q = 0, e = {}, o = [], p = 0, d = {}, l = [], G = null, v = new Image, J = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i, W = /[^\.]\.(swf)\s*$/i, K, L = 1, y = 0, s = "", r, i, h = false, B = b
		...
		e = b.extend({}, b.fn.fancybox.defaults,
				typeof b(a).data("fancybox") == "undefined" ? e : b(a)
						.data("fancybox"));
		w = e.onStart(o, q, e);
		if (w === false)
			h = false;
			...
			if (a.nodeName && !e.orig)
				e.orig = b(a).children("img:first").length ? b(a)
			...
			if (e.type) {
				...
				g = c.match(J) ? "image" : c.match(W) ? "swf" : b(a)
						.hasClass("iframe") ? "iframe" : c.indexOf("#") === 0
						? "inline"
						: "ajax";
			if (g) {
				if (g == "inline") {
		...
		m.wrapInner('<div style="width:'
				...
				+ c
				+ ";overflow: "
				+ (e.scrolling == "auto" ? "auto" : e.scrolling == "yes"
						? "scroll"
		...
		if (f.is(":visible") && false === d.onCleanup(l, p, d)) {
			...
			if (d.overlayShow) {
				...
				if (!u.is(":visible")) {
					M
							&& b("select:not(#fancybox-tmp select)").filter(
									function() {
			...
			if (f.is(":visible")) {
				...
				if (d.transitionIn == "elastic") {
					...
					j.css({
						width : i.width - d.padding * 2,
						height : e.autoDimensions ? "auto" : i.height - y
								- d.padding * 2
					}).html(m.contents());
					f
							.css(i)
			...
			b(document).bind("keydown.fb", function(a) {
				if (a.keyCode == 27 && d.enableEscapeButton) {
					...
					b.fancybox.close()
						...
						&& d.enableKeyboardNav && a.target.tagName !== "INPUT"
		...
		if (d.type == "iframe")
			b('<iframe id="fancybox-frame" name="fancybox-frame'
					+ (new Date).getTime() + '" frameborder="0" hspace="0" '
					+ (b.browser.msie ? 'allowtransparency="true""' : "")
					+ ' scrolling="' + e.scrolling + '" src="' + d.href
					+ '"></iframe>').appendTo(j);
		f.show();
		h = false;
		b.fancybox.center();
	...
	b.fn.fancybox = function(a) {
		...
		b(this).data("fancybox",
				...
				.unbind("click.fb").bind("click.fb", function(c) {
							c.preventDefault();
							if (!h) {
								h = true;
								b(this).blur();
		...
		if (!h) {
			...
			!g && (f.width() > c[0] || f.height() > c[1]) || f.stop().animate({
				top : parseInt(Math.max(c[3] - 20, c[3]
								+ (c[1] - j.height() - 40) * 0.5 - d.padding)),
				left : parseInt(Math.max(c[2] - 20, c[2]
								+ (c[0] - j.width() - 40) * 0.5 - d.padding))
	...
	b.fancybox.init = function() {
		if (!b("#fancybox-wrap").length) {
			...
			b.fn.mousewheel && f.bind("mousewheel.fb", function(a, c) {
						if (h)
							a.preventDefault();
								...
								|| b(a.target).get(0).scrollHeight === b(a.target)
										.get(0).clientHeight) {
							a.preventDefault();
			...
			if (M) {
				...
				b('<iframe id="fancybox-hide-sel-frame" src="'
