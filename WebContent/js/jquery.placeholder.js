/*
 * Placeholder plugin for jQuery --- Copyright 2010, Daniel Stocks
 * (http://webcloud.se) Released under the MIT, BSD, and GPL Licenses.
 */
	...
	d.prototype = {
		...
		hide : function() {
			if (this.valueIsPlaceholder()
					...
					&& (this.input.removeClass("placeholder"), this.input[0].value = "", this.isPassword)) {
				try {
		...
		handlePassword : function() {
			...
			if (b.browser.msie && a[0].outerHTML) {
				var c = b(a[0].outerHTML.replace(/type=(['"])?password\1/gi,
						"type=$1text$1"));
				this.fakePassword = c.val(a.attr("placeholder"))
