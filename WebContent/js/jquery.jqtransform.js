...
(function($) {
	...
	var jqTransformHideSelect = function(oTarget) {
		...
		ulVisible.each(function() {
					var oSelect = $(this)
							.parents(".jqTransformSelectWrapper:first")
							.find("select").get(0);
					// do not hide if click on the label object associated to
					// the select
	...
	var jqTransformReset = function(f) {
		...
		$('a.jqTransformCheckbox, a.jqTransformRadio', f)
	...
	$.fn.jqTransInputButton = function() {
		return this.each(function() {
					var newBtn = $('<button id="' + this.id + '" name="'
							+ this.name + '" type="' + this.type + '" class="'
							+ this.className
	...
	$.fn.jqTransInputText = function() {
		return this.each(function() {
			...
			$input
					...
					.wrap('<div class="jqTransformInputWrapper"><div class="jqTransformInputInner"><div></div></div></div>');
	...
	$.fn.jqTransCheckBox = function() {
		return this.each(function() {
					...
					// Click Handler, trigger the click and change event on the
					// input
					aLink.click(function() {
								...
								// ԭ���$input.trigger('click').trigger("change")���ᷢ����checkbox����2���¼�������
	...
	$.fn.jqTransRadio = function() {
		return this.each(function() {
					...
					aLink.click(function() {
								...
								// uncheck all others of same name input radio
								// elements
								$('input[name="' + $input.attr('name') + '"]',
										inputSelf.form).not($input).each(
										function() {
	...
	$.fn.jqTransTextarea = function() {
		return this.each(function() {
			...
			var oTable = $(strTable).insertAfter(textarea).hover(function() {
				!oTable.hasClass('jqTransformTextarea-focus')
			...
			if ($.browser.safari) {
				$('#jqTransformTextarea-mm', oTable)
						.addClass('jqTransformSafariTextarea').find('div').css(
	...
	$.fn.jqTransSelect = function() {
		return this.each(function(index) {
			...
			var $wrapper = $select.addClass('jqTransformHidden')
			...
			var oLinkOpen = $('a.jqTransformSelectOpen', $wrapper).click(
					function() {
						// Check if box is already open to still allow toggle,
						// but close all other selects
						...
						$ul.slideToggle('fast', function() {
							var offSet = ($('a.selected', $ul).offset().top - $ul
									.offset().top);
							$ul.animate({
										scrollTop : offSet
									});
			...
			var newWidth = (iSelectWidth > oSpan.innerWidth()) ? iSelectWidth
					+ oLinkOpen.outerWidth() : $wrapper.width();
			...
			$.each($("li > a", $ul), function(i, o) {
						$(this).attr("title", $(this).html());
					});

			// oSpan.css({width:iSelectWidth});
			oSpan.css({
						'width' : "75%" // "" + (iSelectWidth - 8 - 20) + "px"  // left-padding:8px, drop-down-arrow-width 20px 
					});
			// Calculate the height if necessary, less elements that the default
			// height
			// show the ul to calculate the block, if ul is not displayed li
			// height value is 0
			$ul.css({
						display : 'block',
						visibility : 'hidden'
