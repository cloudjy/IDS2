...
var parentId= <%=((Category)request.getAttribute("cate")).getParentId()%>;
	$(function(){
	...
	function changedir(callfunc){
		...
		ajaxCallback('filecate!edit' ,
				{"cate.categoryName":$("#dir").val(),
				"cate.categoryId":"<%=((Category)request.getAttribute("cate")).getCategoryId()%>",
	...
	function load(){
		ajaxCallback("filecate!parentCategory", {"id":<%=((Category)request.getAttribute("cate")).getCategoryId()%>}, loadCate);
	}
	var lastID = -1;
	function loadCate(result){
		...
		$(result.data).each(function(i,f) {
			$("#sp").after("<span>-></span><a href=\"javascript:void(0)\" id=\""+f.id+"\""+
	...
	function changeObjectTree(msg){
	 ...
	 function nodeclick(pid, obj){
		 parentId = obj.id;
		cateId = obj.id;
		 var selfid = <%=((Category)request.getAttribute("cate")).getCategoryId()%>;
		 var groupSelector = $("#groupSelector");
		 if ($(obj).is('select')) {
		 ...
		 ajaxCallback("filecate!listNotThis?cate.categoryId=<%=((Category)request.getAttribute("cate")).getCategoryId()%>",param,showObjectList,obj);
		 return false;			
	}
	...
	<form action="#" method="post" class="niceform">
	    <table width="100%" border="0" cellspacing="2" cellpadding="0" class="bdTab">
 		<tr>
			...
			<td>
			<input type="text" name="dir" id="dir" value="<%=((Category)request.getAttribute("cate")).getCategoryName()%>"/>
