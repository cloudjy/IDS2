...
<script type="text/javascript">
	...
	<% 
		int categoryId = ((Category)request.getAttribute("category")).getCategoryId();
	...
	function dirClick(pid, obj) {
		var selfid =<%= categoryId %>;
		...
		$.ajax({
			type : "post",
			dataType : "json",
			url : "<%= _actionName %>!categorySelector",
			data : param,
			global : false,
			complete : function() { 
			},
				...
				if (msg.success) {
					try {
						...
						if ($(obj).is('select')) {
							...
							groupSelector.html(""); 
							groupSelector.before("<a href=\"javascript:void(0)\" " + 
								"id=\"" + oldId + "\" " + 
								"onclick=\"dirClick(" + oldId + ",this)\" >" + oldText + "</a>" + 
								"<span id=\""+oldId+"s\">-></span>");

							if (msg.data != null && msg.data.length > 0) {
								var optionHtml = ""; 
								optionHtml += "<option value=\"0\"><s:text name="select" /></option>";
								$.each(msg.data, function(i, g) {
									optionHtml += "<option value=\""+g.categoryId+"\">" + g.categoryName + "</option>";
								});
							...
							var optionHtml = "";
							if (msg.data != null && msg.data.length > 0) {
								optionHtml += "<span>-></span><div style=\"display:inline-block;\">";
								optionHtml += "<select size=\"1\" name=\"groupSelector\" onchange=\"dirClick(0,this)\" id=\"groupSelector\">";
								optionHtml += "<option value=\"0\"><s:text name="select" /></option>";
								$.each(msg.data, function(i, g) {
					...
					<table border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							...
							<td style="width: 500px; white-space: nowrap;">
								<span id="sp"> 
									<a class="root" href="javascript:void(0)" id="0" onclick="dirClick(0,this)"><s:text name="root" /></a> 
									<%
								    	...
								    	for(Category g : categorypath) { 
								    		if(g.getCategoryId()>0){ 
								            	...
								            	out.print("<a href=\"#\" id=\""+g.getCategoryId()+"\" " + 
								            		"onclick=\"dirClick("+g.getCategoryId()+",this)\" >"   + g.getCategoryName() + 
								            	   	"</a>");          
								    		}
								       	}
								    }
					                ...
					                if(brother != null && brother.size()>0) {                	
					                	 ...
					                	 out.print("<select size=\"1\" name=\"groupSelector\" onchange=\"dirClick(0,this)\" id=\"groupSelector\">");
			...
			<div>
				...
				<div style="width: 590px; float: right; height: 440px;">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							<td class="first" style="width: 200px; white-space: nowrap;"><s:text name="name" /></td>
							...
							<!--  
							<td style="width: 60px; white-space: nowrap;"><s:text name="task_play_effect" /></td>
							<td style="width: 70px; white-space: nowrap;"><s:text name="task_play_effecttime" /></td>
							<td><s:text name="task_play_scalestyle" /></td>
							 -->
						</tr>
					</table>
					...
					<div class="rightMenu"
						...
						<ul class="left rightMenuShort">
							<li><a
								onclick="return CheckHelper.checkAll('flashfileid');return false;"
								class="CheckLink"><s:text name="selectall" /></a></li>
							<li><a
								onclick="return CheckHelper.reverseAll('flashfileid');return false;"
								class="CheckLink"><s:text name="invertselection" /></a></li>
							<li><a
								onclick="return CheckHelper.cleanAll('flashfileid');return false;"
								class="CheckLink"><s:text name="cancel" /></a></li>
...
<script type="text/javascript">
	...
	$(function() {
		...
		var url = "<%= _actionName %>!querycontentlist"; 
			...
			success : function(msg) {
				try {
					...
					$.each(msg.data, function(i, f) {
						...
						content += "<a href=\"file!preview?file.fileId=" + f.fileId + "\" target='_blank' >";
						...
						content += "<input type=\"checkbox\" value=\""+f.fileId+"\" name=\""+f.fileId+"\" id=\"" 
							+  f.fileId
							+  "\" class=\"fileid\" />";
						content += "<span>" + (f.fileName.length > 8 ? f.fileName.substr(0, 8) + "..." : f.fileName) + "</span>  ";
						content += "</li>";
	...
	function addfile() {
		...
		if (ok > 0) {
			...
			ajaxCallback("<%= _actionName %>!addFlashFiles", param, function(msg) {
				if (msg.success) {
					queryflashfiles();
				} else {
					alert(msg.data);
	...
	function queryflashfiles(remember) {
		var param = {
			"taskID" : <%= request.getAttribute("taskID") %> ,
			"programID" : <%= request.getAttribute("programID") %> ,
			"rectID" : <%= request.getAttribute("rectID") %> 
	...
	function showlist(result) {
		...
		$.each( result.data, function(i, f) {
			...
			content += "<tr id=\""+f.contentId+"\">"
				...
				//		.substr(0, 8)
				//		+ "..." : f.fileName)
				+ "<ul style=\"width:200px;overflow:hidden;text-overflow:ellipsis;\"><nobr>"
				+ f.fileName
				+ "</nobr></ul>"
				+ "</td>"
				+ "<td style=\"width:60px;\">"
	...
	function elementsOperate(op) { 
		...
		var arr = $("input[class='flashfileid']"); //document.getElementsByTagName('input'); 
		var selectIDs = ""; 
		var fileIDs = "";
		for ( var i = 0; i < arr.length; i++) {
			var abc = arr[i];
		...
		if (op == "delete") { 
			...
			var modifyFlashUrl = opUrl + 
					"?taskID=" + <%= request.getAttribute("taskID") %>  + 
					"&programID=" + <%= request.getAttribute("programID") %> + 
					"&rectID=" + <%= request.getAttribute("rectID") %> + 
					"&fileIDs=" + fileIDs.substring(0, fileIDs.length - 1) + 
					"&selectIDs=" + selectIDs.substring(0, selectIDs.length - 1); 
			showIPUBDialogStand(modifyFlashUrl, {title:"<s:text name="title_edit_contents_attribute"/>", width : 564,height : 377}); 
