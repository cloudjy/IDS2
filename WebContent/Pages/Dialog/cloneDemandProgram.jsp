...
<%
DemandProgram demandprogram = (DemandProgram)request.getAttribute("demandprogram");
%>
...
<script type="text/javascript">
		function changedir(callfunc){	
			...
			/*   var playcount = 0;	
	        	if($("#playCount").val() != null && $("#playCount").val() != '' && $("#playCount").val() != undefined)
	        		playcount =  $("#playCount").val();	 */ 
			
	      $.post('demandprogram!Clone',
					{"demandprogram.programName":$("#programName").val(),
			         "demandprogram.description":$("#description").val(),
					 "demandprogram.taskId":$("#taskid").val(),
					 "demandprogram.programId":$("#programid").val(),
					// "demandprogram.playCount":playcount,
					// "demandprogram.playStyle":$("#playStyle").val(),
					 "purpose":$("#purpose").val()
		            },
				function(txt) {
			     callfunc(txt);
...
<body> 
  ...
  %><!-- 弹出窗口 开始-->
    ...
    <input type="hidden" value="<%=demandprogram.getTaskId() %>" id="taskid" name="demandprogram.taskId" /> 
    <input type="hidden" value="<%=demandprogram.getProgramId() %>" id="programid" name="demandprogram.programId" />	
	<table width="100%" border="0" cellspacing="2" cellpadding="0"
					class="bdTab">
		<tr>
			<td class="bt" style="width: 40%"><font color="#FF0000" size="3">&#x25B4;</font><s:text name="pname" /></td>
			<td align="left"><input type="text" name="programName" id="programName" /> 
		...
		<tr>
			...
			<td align="left"><select size="1"  id="purpose" name="purpose">	
				...
				%>				
					<option <%if(demandprogram.getTaskId()==list.get(i).getTaskId()){%> selected <%} %> value="<%=list.get(i).getTaskId() %>"><%=list.get(i).getTaskName() %></option>
				<%}
			  } %>
			</select>
			</td>
