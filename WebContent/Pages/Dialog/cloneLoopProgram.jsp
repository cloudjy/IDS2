...
<%@ page language="java" import="com.gnamp.server.model.LoopProgram"%>
<%@ page language="java" import="com.gnamp.server.model.LoopTask"%>
<%@ taglib uri="/struts-tags" prefix="s"%>

<%
LoopProgram loopprogram = (LoopProgram)request.getAttribute("loopprogram");
%>
...
<body> 
  ...
  %>
    ...
    <input type="hidden" value="<%=loopprogram.getTaskId() %>" id="taskid" name="loopprogram.taskId" /> 
    <input type="hidden" value="<%=loopprogram.getProgramId() %>" id="programid" name="loopprogram.programId" />	
	<table width="100%" border="0" cellspacing="2" cellpadding="0"
		...
		<tr>
			...
			<td align="left"><select size="1" id="purpose" name="purpose">	
				...
				%>				
					<option <%if(loopprogram.getTaskId()==list.get(i).getTaskId()){%> selected <%} %> value="<%=list.get(i).getTaskId() %>"><%=list.get(i).getTaskName() %></option>
				<%}
			  } %>
			</select>
			</td>
