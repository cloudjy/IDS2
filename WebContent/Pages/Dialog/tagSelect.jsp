...
<%@ page language="java" import="com.gnamp.struts.filter.Context"%>
...
<script type="text/javascript">
	...
	<% 
	int REQUEST_SOURCE_TYPE = SourceType.RSS;
	try {
		REQUEST_SOURCE_TYPE = Integer.parseInt(request.getAttribute("type").toString());
	} catch (Exception e) {
		REQUEST_SOURCE_TYPE = SourceType.RSS;
	}
	%>
	var REQUEST_SOURCE_TYPE = <%= REQUEST_SOURCE_TYPE %>;
    ...
    $(function() {
       ...
       /*  $("#namesort").bind("click", namesort);
        ...
        <% if (Context.showBuiltinResource() && REQUEST_SOURCE_TYPE == SourceType.RSS) { %>	 
        	$("#select").val("2");
  ...
  function globeParam2(inparam) {
	    var param = {"id": $("#ddllast").val(),"internaltag.name":""};
	    if (typeof(inparam) != undefined) {
	        copyParam(param, inparam);
	    }
	    return param;
  ...
  function sortfield2(flag, name) {
	    $internal.currentSortManaer = flag;
	    $internal.currentSortName = name;
	    $internal.ajax($internal.globeParam(globeParam2()));
	}
  function showlistfunc(result) {
	    try {
	        ...
	        var content = "";
	        var thead = "<tr>"  
	        + "<td  class=\"first\" style=\"width:80px;\">" + langMap.tagMap.state + "</td>" 
	        + "<td id=\"namesort\" onclick='namesort()' style=\"cursor: pointer; width:380px;\" >" + langMap.fileMap.fileName + "</td>" 
	        + "<td id=\"idsort\"  style=\"width:180px;\" >" + "ID" + "</td>" 
	        + "<td id=\"typesort\" >" + langMap.tagMap.type + "</td>" 	        
	        + "</tr>";
	        $.each(result.data, function(i, f) { 
	        	if (REQUEST_SOURCE_TYPE != f.type) {
  		        	return;
	        	}
  ...
  var duma = {
  	...
  	function systagchange(){
  		...
  		//加载下拉框
  		if($("#ddlsystag").val()=="0"){
  			 var optionhtml = "<option value=\"0\"><s:text name="select" /></option>";
  			$("#ddllast").html(optionhtml);
  		}
  	}
  	
  	function loadimternal(){
  		$internal = new page("internal!query", {"id": $("#ddllast").val(),"internaltag.name":""}, function(result){
  	...
  	function loadddl(){
  		...
  		ajaxCallback("internal!list", {"id": $("#ddlsystag").val(),"internaltag.name":""}, function(result){
  			ddlfunc(result);
  		});
  	}
  	
  	function ddlfunc(result) {
  	    try {
  	        var optionhtml = "";
  	...
  	function showinternalfunc(result) {
  	    try {
  	        ...
  	        var content = "";  	      
  	        var thead = "<tr>"  + 
/*   	        "<td  id=\"sizesort\" class=\"first\" onclick='sizesort()'  style=\"cursor: pointer;width:80px;\">" + getNameCategory() + "</td>" + */
  	        "<td id=\"namesort\" class=\"first\" onclick='namesort2()' style=\"cursor: pointer; width:230px;\" >" + langMap.tagMap.name + "</td>" +
  	        "<td id=\"contentsort\" style=\"width:230px;\" >" +  langMap.tagMap.content + "</td>" + "</tr>";
  	        if(result!=undefined&&result.data!=undefined){
  		        $.each(result.data, function(i, f) {
  		            ...
  		            content += "<tr>";
  		            content +="<td style=\"width: 10px;\"><input type=\"checkbox\" class=\"sourceid\" name=\"selectsource\" id=\"" + f.name + "\" value=\"" + f.sourceId + "\" /></td>";
   		            content += "<td style=\"width:210px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;\" title=\""+f.name+"\">" + f.name + "</td>" ; 
/*   		            content += "<td style=\"width:180px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;\">" + f.BasePrice + "</td>" ;
  		            content += "<td style=\"width:180px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;\">" + f.fBuyPrice + "</td>" ; */
  		            content += "<td style=\"width:250px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;\"><a class='blue1' href='javascript:selectTextContext("+f.sourceId+","+f.catId+")'>"+langMap.tagMap.view+"</a></td>";
  		            content += "</tr>";
  		        });
  	        }
  	        $("#shujBt").remove();
  	...
  	function selectTextContext(sourceid,catid){
  		showIPUBDialogStand("internal!select_text.action?internaltag.sourceId="+sourceid+"&internaltag.catId="+catid, {"title":langMap.globalMap.viewText,width:470,height:280, resizable: false});
  	}
  	function ddllastchange(){
  		loadimternal();  
  	}
</script>
</head>
			...
			<form method="post" class="niceform">
				<div class="tcBt2">
					<table border="0" cellspacing="0" cellpadding="0" style="table-layout: fixed;">
						<tr>
							<td style="width:50px;">&nbsp;<s:text name="tag" /></td>
							<td style="width:100px;">
							<select onchange="selectchange();" size="1" name="select" id="select"
							...
							<span style="display: none;" id="sp2">
							<s:text name="internal" />->
							 <select size="1" id="ddlsystag" onchange="systagchange();"
							   name="ddlsystag">
								<%
									if (request.getAttribute("systemsourcecates") != null) {
										List<CategoryXMLConfig> list = (List<CategoryXMLConfig>) request.getAttribute("systemsourcecates");
			
										for (int i = 0; i < list.size(); i++) {
								%>
							<option title="<%= list.get(i).getName()%>" value="<%=list.get(i).getId() %>" ><%=(list.get(i).getName().length()>10 ? (list.get(i).getName().substring(0, 10)+"...") : list.get(i).getName()) %></option>
								<%
									}
									}
								%>			
			   			 </select>-&gt;
			   			 <select size="1" id="ddllast" onchange="ddllastchange();"
							   name="ddllast">
								<option value="0">
									<s:text name="select" />
								</option>	
							...
							<td style="width:20px;" valign="middle" align="right">
								<span id="check">
							 		<input type="checkbox" onchange="query()" id="includeChildren" value="true" /> 
								</span>
							</td> 
							<td style="width:115px;">
								<span id="child"><s:text name="child" /></span>
							</td>
						</tr>
					</table>
				</div>
