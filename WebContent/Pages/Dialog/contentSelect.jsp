						...
						if (msg.success) {
							try {
								...
								if ($(obj).is('select')) {
									...
									groupSelector
											.before("<a href=\"javascript:void(0)\" id=\""
													+ oldId
													+ "\" onclick=\"nodeclick("
													+ oldId
													+ ",this)\" >"
		...
		<div class="tcNr">
				...
				<div class="tcBt2">
					<table border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							...
							<td>							
							<select onchange="query();" size="1" name="select" id="select">
							     <option value="0">
										<s:text name="image" />
								</option>
							     <option value="1">
							...
							<span id="sp"> <a
									class="root" href="javascript:void(0)" id="0" onclick="nodeclick(0,this)"><s:text
											name="root" /></a> <%
			...
			<div>
				...
				<div style="width: 590px; float: right; height: 440px;">
					<table id="rightbt" width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							...
							<td style="width: 70px;"><s:text name="task_play_scalestyle" /></td>
							<td style="width: 70px;"><s:text name="task_play_effect" /></td>
							<td style="width: 70px;"><s:text name="task_play_effecttime" /></td>
							<td style="width: 70px;"><s:text name="rotation" /></td>
						</tr>
					</table>
...
<script type="text/javascript">
							...
							$.each(
											...
											function(i, f) {
														...
														content += "<tr id=\""
															...
															+ (f.fileName.length > 10 ? f.fileName
																	.substr(0, 10)
																	+ "..."
																	: f.fileName)
															+ "</td>"
	...
	$(function(){
		 var param = {"taskID" :<%= request.getAttribute("taskID") %>,
				...
				var surl = "";
				if (<%=request.getAttribute("taskType").toString().equals("loop") %>) {
					surl = "looprect!querycontentfiles";
				}
				if (<%=request.getAttribute("taskType").toString().equals("demand") %>) {
					surl = "demandrect!querycontentfiles";
				}
				if (<%=request.getAttribute("taskType").toString().equals("plugin") %>) {
					surl = "pluginrect!querycontentfiles";
				}
	...
	function addfile() {
		...
		if (ok > 0) {
			...
			var surl = "";
			if (<%=request.getAttribute("taskType").toString().equals("loop") %>) {
				surl = "looprect!addcontentfile";
			}
			if (<%=request.getAttribute("taskType").toString().equals("demand") %>) {
				surl = "demandrect!addcontentfile";
			}
			if (<%=request.getAttribute("taskType").toString().equals("plugin") %>) {
				surl = "pluginrect!addcontentfile";
			}
	...
	function showlist(result){
		...
		+ " id=\"" + f.contentId + "\" class=\"imagefileid\" />"
		+ toHtml(f.fileName)
		+ "</td>"
		+ "<td style=\"width:50px;\">"
		+ f.sequenceNum
		+ "</td>"
		+ "<td style=\"width:50px;\">"
		+ (f.contentType==1 ? f.playTime : (f.playTime > 0 ? f.playTime : ""))
		+ "</td>"
		+ "<td style=\"width:70px;\" title=\"" + toHtml(getScaleStyle(f.scaleStyle)) + "\">"
		+ toHtml(getScaleStyle(f.scaleStyle))
		+ "</td>"
		+ "<td style=\"width:70px;\" " + (f.contentType==1 ? ("title=\"" + toHtml(getSwapStyle(f.swapStyle)) + "\"") : "") + "\">"
		+ (f.contentType==1 ? toHtml(getSwapStyle(f.swapStyle)) : "")
		+ "</td>"
		+ "<td style=\"width:70px;\">"
		+ (f.contentType==1?f.swapTime:"") 
		+ "<td>"
		+ "<td style=\"width:70px;\">"
		+ (f.contentType!=1?(f.rotation > 0 ? "<s:text name="off" />" : "<s:text name="on" />"):"")
		+ "<td>" 
		+ " </tr>";		
		});
	...
	function elementsOperate(op) {
		var opArray = { 
			...
			"edit":"<%= _actionName %>!ToModifyContent", 
			"up":"<%= _actionName %>!up", 
