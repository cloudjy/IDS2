...
$(function() {
	...
	function changedir(callfunc){	
	    ...
	    if(!istemplet)	{ 
	    	$.post('demandprogram!Add', {
		    		"demandprogram.programName":$("#programName").val(),
					"demandprogram.description":$("#description").val(),
					"demandprogram.width":w.val(),
					"demandprogram.height":h.val(),
					"demandprogram.playCount": (playcount && playcount > 0? playcount : 1) ,
					"demandprogram.playStyle": (playcount && playcount > 0? 1 : 0), // $("#playStyle").val(), 0-count, 1-duration
					"istemplet":istemplet,
					"isbuildin":isbuildin,
					"demandprogram.taskId":$("#taskid").val()
				},
		    	function(txt) { 
		    		callfunc(txt);
		    	}
	...
	<div class="tcWindow">
		<div class="tcBt">
			<img src="${theme}/skins/default/images/tcBt1.gif" width="5" height="35"
				class="left" />
			<h2><s:text name="text_current" /><s:text name="text_demandtask" />:&nbsp;<%= task.getTaskName() %></h2>
			<img src="${theme}/skins/default/images/tcBt3.gif" class="right" />
		</div>
		<div class="tcNr">
			<form action="#" method="post" class="niceform">
 <input type="hidden" value="<%=task.getTaskId() %>" id="taskid" name="demandprogram.taskId" />	
 ...
 <input class="cssH" name="height" id="height" type="hidden" />
	<table width="100%" border="0" cellspacing="2" cellpadding="0"
		...
		<tr style="display:none;">
			...
			<td align="left"> <select size="1" id="playStyle" name="playStyle">
                  <option value="0"><s:text name="text_adddemandprogram_count" /></option>
                  <option value="1"><s:text name="text_adddemandprogram_duration" /></option>
               </select>
            </td>
		</tr>
		<tr>
				...
				<table border="0" cellspacing="0" cellpadding="0" >
					<tr>
						<td style="padding-left: 0px;padding-right: 3px;">
						 	<input type="radio" name="idle" id="idle_awhile" 
						 		checked  style="margin: 0;" onchange="onIdleAwhileRadioChange()"/>
						</td>
						<td style="padding-left: 0px;">
						 	<s:text name="text_idle_playtime1"/>
						</td>
						<td style="padding-left: 0px;">
							 <input id="playCount" name="playCount" value="30" maxLength="5" 
		...
		<tr>
			...
			<td align="left">  
             	<table border="0" cellspacing="0" cellpadding="0" >
					<tr>
						<td style="padding-left: 0px;padding-right: 3px;">
						 	<input type="radio" name="idle" id="idle_forever" 
		...
		<tr>
			...
			<td align="left"> <select size="1" id="fbl" onchange="resolutionChanged()">
