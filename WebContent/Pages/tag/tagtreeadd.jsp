...
<%
	try{
		if(request.getAttribute("id")==null||Integer.valueOf(request.getAttribute("id").toString())<0){
			throw new Exception();
		}
	}catch(Exception e){
		return;
...
%>
	function changedir(callfunc){ 
		...
		ajaxCallback('tagcate!add' ,
				{"source.categoryName":$("#dir").val(),
				"source.parentId":"<%=request.getAttribute("id")%>"
				},
				function(txt){
					callfunc(txt);
				}
	...
	function executeFunction(result){
		if(result.success){
			callParentFunction("treeInternal.addNoded", result.data);
			return closeIFrame();
		}else{
