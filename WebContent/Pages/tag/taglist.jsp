<%@page import="com.gnamp.struts.vo.CategoryXMLConfig"%>
		...
		<div class="bt1">
				<ul class="tab1">
					<li onclick="location.href='file.action'"><s:text name="file"/></li>
               		<li onclick="location.href='tag.action'" class="hover"><s:text name="custom"/></li>
                	<li onclick="location.href='templet.action'"><s:text name="templet" /></li>
           		 </ul>
           		 <form class="topSearchMk2 topSearchMk3" style="margin-top: -10px;">
				<table class="tagFilter" width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                <td style="width:58%"></td>
                    <td class="search" style="width:145px; overflow:hidden;">
					    <div class="searchL">
					   		<input id="search" name="tagName" type="text" style="width:110px;"/>
					    </div>
					     <div class="searchR" style="float:left">
						     <button onclick="queryfiles();return false;" >
						     </button>
					     </div>
                    </td>
                    <td width="5%">
            ...
            <table class="internalFilter" width="100%" border="0" cellspacing="0" cellpadding="0" style="display:none">
                <tr>
                	<td style="width: 489px"></td>
					<td style="width: 54px;text-align:right;"><s:text name="tagtype"/></td>
                    <td style="width: 77px;">
                        <select id="internalType" name="internal.type"  onchange="internalreload()">
                        	<%
                        		for(CategoryXMLConfig configx : Context.getSystemSourceCates()){
                        			out.print("<option value="+configx.getId()+">"+configx.getName()+"</option>");
                        		}
                        	%>
						</select>
					</td>
					<td style="width: 12px"></td>
                    <td class="search" style="width:158px; overflow:hidden;">
					    <div class="searchL">
					    <input id="search" name="searchInternal" type="text" style="width:110px;"/></div>
					    <div class="searchR"><button onclick="internalreload();return false;" ></button></div>
			     	</td>
			...
			<div class="left1">
				...
				<div class="leftMenu" >
					...
					<div class="leftMenuBta">
					 <div id="InternalRss_Bar" style="cursor: pointer;" onclick="internalreload()"><s:text name="internaltag" /></div>
					</div>
					 <%} %>
					<div class="leftMenuBta">
					 <div id="CustomRss_Bar"  style="cursor: pointer;" onclick="selectLoad()"><s:text name="customtag" /></div>
					</div>
				...
				<div class="leftButtonBox" style="margin-top: -75px;">
					<a href="javascript:void(0)" class="addTagNode"><s:text name="create" /></a>
					<a href="javascript:void(0)" class="editTagNode"><s:text name="edit" /></a>
					<a href="javascript:void(0)" class="removeTagNode"><s:text name="delete" /></a>
			...
			<div class="right1">
				...
				<div class="rightMenu" style="height:20px;">
					<ul class="left rightMenuLong">
						<s:if test="#session.current_privilege.uploadfile">
						<li id="add"><font color="#FF0000" size="3">&#x25B4;</font><s:text name="create"/></li>
						</s:if>
						<s:if test="#session.current_privilege.editfile">
						<li id="edit"><s:text name="edit"/></li>
						</s:if>
						<s:if test="#session.current_privilege.delfiles">
						<li id="delete"><s:text name="delete"/></li>
						</s:if>
						<li id="exportTagNode"><s:text name="export"/></li>
					</ul>
				</div> 
	            <div style="font-size:12px;padding-left:20px;display:none;" id="bottom_prompt_custom"><s:text name="bottom_prompt_rss_custom"/></div> 
	            <div style="font-size:12px;padding-left:20px;display:none;padding-top:40px;" id="bottom_prompt_internal"><s:text name="bottom_prompt_rss_internal"/></div> 
...
<script type="text/javascript">
	$(function () { 
		<% if (Context.showBuiltinResource()) { %> 
		$("#InternalRss_Bar").trigger("click");
		<% } else { %>
		$("#CustomRss_Bar").trigger("click");
		<%} %> 
	});
</script>
