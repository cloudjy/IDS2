...
duma.BeautifyScrollBar = function(obj, arrowUpCss, arrowUpActiveCss, handleCss,
		handleActiveCss, arrowDownCss, arrowDownActiveCss) {
	...
	this.obj.innerHTML = '<div id="'
			+ obj
			+ 'Area" class="dumascroll_area">'
			+ this.obj.innerHTML
			+ '</div><div id="'
			+ obj
			+ 'Bar" class="dumascroll_bar"><div class="dumascroll_arrow_up"></div><div class="dumascroll_handle"></div><div class="dumascroll_arrow_down"></div></div>';
	...
	this.handleHeight = parseInt(this.area.offsetHeight
			/ this.area.scrollHeight
			* (this.bar.offsetHeight - this.arrowUpHeight - this.arrowDownHeight));
};
duma.BeautifyScrollBar.prototype = {
	setObjCss : function() {
		duma.getStyle(this.obj).position == "static"
				? this.obj.style.position = "relative"
				: duma.getStyle(this.obj).backgroundColor == "transparent"
						? this.obj.style.backgroundColor = "#fff"
						: null; // 若容器本来就没有设position，则初始化为relative；若容器原来未设置背景色，则初始化为白色；
	},
	sethandle : function() { // 当内容超多时设置拖拽条子的最小高度
		this.handle.style.top = this.arrowUpHeight + "px";
		if (this.handleHeight > this.handleMinHeight) {
			this.handleHeight < this.bar.offsetHeight - this.arrowUpHeight
	...
	areaScroll : function() {
		/*
		 * this.handle.style.top = this.handle.style.display != "none" ?
		 * this.area.scrollTop/(this.area.scrollHeight - this.area.offsetHeight) *
		 * (this.bar.offsetHeight - this.handleHeight - this.arrowUpHeight -
		 * this.arrowDownHeight) + this.arrowUpHeight + "px" : null;
		 */
		// 注释后ie8兼容
		// this.handle.style.top = this.area.scrollTop/(this.area.scrollHeight -
		// this.area.offsetHeight) * (this.bar.offsetHeight - this.handleHeight
		// - this.arrowUpHeight - this.arrowDownHeight) + this.arrowUpHeight +
		// "px";
	},
	areakeydown : function(e) { // 支持键盘上下按键
		...
		document.onkeydown = function(event) {
			...
			if (that.area.scrollTop > 0
					&& that.area.scrollTop < that.area.scrollHeight
	...
	handleMove : function(e) {
		...
		this.area.scrollTop = (e.clientY - this.relY - this.arrowUpHeight)
				/ (this.bar.offsetHeight - this.handleHeight
						- this.arrowUpHeight - this.arrowDownHeight)
				* (this.area.scrollHeight - this.area.offsetHeight);
		this.setBarPos();
	},
	...
	barDown : function(e) {
		var e = e || window.event, that = this, eY = e.clientY, mStep = this.bar.offsetHeight, documentScrollTop = document.documentElement.scrollTop
				|| document.body.scrollTop, hOffset = duma
				.getOffset(this.handle), bOffset = duma.getOffset(this.bar);
		if (documentScrollTop + eY < hOffset.top) {
			this.barMouseDownInterval = setInterval(function(e) {
				...
				if (that.area.scrollTop <= (eY + documentScrollTop
						- bOffset.top - that.arrowUpHeight)
						/ (that.bar.offsetHeight - that.arrowUpHeight - that.arrowDownHeight)
						* that.area.scrollHeight) {
			...
			this.barMouseDownInterval = setInterval(function() {
				that.area.scrollTop += that.area.offsetHeight;
				if (that.area.scrollTop >= (eY + documentScrollTop
						- bOffset.top - that.arrowUpHeight - hOffset.height)
						/ (that.bar.offsetHeight - that.arrowUpHeight - that.arrowDownHeight)
						* that.area.scrollHeight) {
					that.clearBarMouseDownInterval();
				}
...
duma.BeautifyScrollBar.init = function() {
	var o = document.getElementsByTagName("div"), oLen = o.length, dumascrollClass = /\bdumascroll\b/, oArr = [], oArrLen = oArr.length;
	...
	for (var j = 0; j < oArrLen; j++) {
		new duma.BeautifyScrollBar(oArr[j], "dumascroll_arrow_up",
				"dumascroll_arrow_up_a", "dumascroll_handle",
				"dumascroll_handle_a", "dumascroll_arrow_down",
				"dumascroll_arrow_down_a").run();
	}
}
