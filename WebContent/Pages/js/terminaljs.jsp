...
function curtaskstatesort() {
	taskstateflag = (taskstateflag == "asc" ? "desc" : "asc");
	sortfield(taskstateflag,"LOOPTASK_UPDATED " + taskstateflag + ", " + 
			"DEMANDTASK_UPDATED " + taskstateflag + ", PLUGINTASK_UPDATED"); //, DEMANDTASK_UPDATED, PLUGINTASK_UPDATED 
}
...
function showlist(result) {
	try {
		...
		tbthtml+="<td onclick=\"curtaskstatesort();\" id=\"taskstate\" style=\"cursor: pointer; width: 50px;\"><s:text name="taskstate" /></td>";
		// tbthtml+="<td id=\"softwarestate\" style=\"cursor: pointer; width: 50px;\"><s:text name="softwarestatus" /></td>";
		tbthtml+="<td id=\"setcity\" style=\"cursor: pointer; width: 50px;\"><s:text name="setcity" /></td>";
		       ...
		       $.each(
						...
						function(i, f) {
							...
							content += "<tr style=\"word-break: keep-all\" id=\""
									...
									// + "</td>"
									+ "<td style=\"width: 50px;\">"
									+ escapeJQueryDiv.text(f.cityName).html() 
									+ "</td>"
									+ "<td style=\"width: 50px;\">"
