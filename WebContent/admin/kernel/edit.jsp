...
<%@ page import="com.gnamp.server.model.Kernel" %>
...
var version = "<%=((Kernel)request.getAttribute("kernel")).getKernelVersion()%>";
	$(function(){
		load();
	});
	function submitForm(callfunc){
		ajaxCallback('kernel!edit' ,
				{"kernel.kernelVersion":version,
				"kernel.kernelName":$("input[name=kernelName]").val(),
				"kernel.description":$("input[name=description]").val(),
				},
				function(txt){
					callfunc(txt);
				}
	...
	function load(){
		ajaxCallback("kernel!searchBean", {"kernel.kernelVersion":version}, loaded);
	}
...
<div class="tcWindow">
	...
	<form action="#" method="post" class="niceform">
	    <table width="100%" border="0" cellspacing="2" cellpadding="0" class="bdTab">
 		<tr>
			<td class="bt"><s:text name="version"/></td>
			<td><span id="kernelVersion"></span></td>
		</tr>
		<tr>
