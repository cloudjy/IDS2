   ...
   extends BaseAction
 {
   ...
   public void upload()
   {
     ...
     try
     {
       ...
       int file_flag = UploadUtils.getFileFlag(this.upload0FileName);
       if (!com.gnamp.server.model.File.validType(file_type)) {
         throw new IllegalArgumentException("invalid file extension");
       }
       if ((this.categoryId > 0) && 
         (new CategoryHandle(this).read(this.categoryId) == null)) {
         throw new IllegalArgumentException("category[" + this.categoryId + "] not exist");
       }
       if ((this.jufinal < 0) && (this.jupart < 0))
       {
       ...
       if ((this.jufinal > 0) || ((this.jufinal < 0) && (this.jupart < 0)))
       {
         ...
         com.gnamp.server.model.File file = fileHandle.upload(
           getWholeFilePath(), 
           this.upload0FileName, this.upload0FileName, 
           this.categoryId, file_type, "", file_flag);
         logEvent(getEnglish("uploadfile"), getEnglish("uploadfile") + ":[" + this.upload0FileName + "]");
         if (this.autoCheck) {
 ...
 * Qualified Name:     com.gnamp.struts.action.FileUploadAction
